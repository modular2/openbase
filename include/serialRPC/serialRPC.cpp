#include "serialRPC.hpp"
#include <iostream>
#include <iomanip> 
using namespace std;
#define    DEFAULT_BAUD_RATE   9600
 serialPort serial;
      const  uint8_t auchCRCHi[256]={
    
};
const  uint8_t auchCRCLo[256]={
    
};
 serialRPC::serialRPC() 
			{
			_method=-1;
			}
 bool serialRPC::init(const char * dev)
 {            cout<<"seerial init:"<< endl;
	         serial.OpenPort(dev);
             cout<<"serial Setup"<< endl;		             
	   return  serial.setup(19200,0,8,1,'N');    
 
			 
 } 
 bool serialRPC::callMethod(uint8_t address,uint8_t method,uint8_t *data,int length )
 {
	  int index,i;
    index=0;
	 _address=address;
     _method=method;
     sbuf[index++]=0x55;
     sbuf[index++]=0xaa;
   //  sbuf[index++]=0x55;
   //  sbuf[index++]=0xaa;
    sbuf[index++]=address;
    sbuf[index++]=method;
    sbuf[index++]=(length>>8)&0xff;
    sbuf[index++]=length&0xff;
	for (i=0;i<length;i++)
		sbuf[index++]=data[i];
    unsigned short resultCRC =  calculateCRC16(&sbuf[2],(index-2)); 
    sbuf[index++]=(resultCRC>>8)&0xff;
    sbuf[index++]=resultCRC&0xff; 
    // sbuf[index++]=0x55;
    // sbuf[index++]=0xaa;
    // sbuf[index++]=0x55;
   //  sbuf[index++]=0xaa;
  // for (i=0;i<index;i++)
  //  cout<<"0x"<<hex<<setfill('0')<<std::setw(2)<<unsigned (sbuf[i])<<",";
  //  cout<<endl;
	int len =serial.writeBuffer(sbuf, index);
  cout <<"len="<<len<<"index="<<index<<endl;
    return true;
 }
 int serialRPC::waitResult(uint8_t * data)
 {
	   unsigned short resultCRC ,recCRC;
    
    int state,index,i;
	uint16_t len,cnt;
    uint8_t ch;
    bool flag;
     flag=true;
     state=0;
     index=0;
     while(flag)
     { 
         ch=serial.getchar();
     //  cout<<"0x"<<hex<<setfill('0')<<std::setw(2)<<unsigned(ch)<<endl;
         switch(state)
         {   case 0:{
                    
                if (ch==0x55) 
                  state=1;
                 break;
                 }
             case 1:{
                    
                if (ch==0xAA) 
                  state=2;
                  else 
                  state=0;
                 break;
                 } 
             case 2:{
                   if (ch==0x55) 
                  state=1;
                  else {
                      state=3;
                      _address=ch;
                      rbuf[index++]=_address; 
                      }
                 break;
                 }
            case 3:{
               
               _method=ch;
                   state=4;
                   rbuf[index++]=ch;                  
                 break;
                 }
             case 4:{
                    len=ch;
                    rbuf[index++]=ch;
                    state=5;
                    break;
                    } 
			case 5:{
				   len=((len<<8)&0xff00)|ch;
           if (len>256) len=0;
          
					if (len>0)
                {
                 cnt=len;
				         rbuf[index++]=ch;
                  state=6;
                  }    
			      	else 
				     	 state=7;
                break;
		        	}		
             case 6:{
                    rbuf[index++]=ch;
                    cnt--;
                    if (cnt==0)
                      { 
                      state=7;
                     
                      }
                    break;
                    }
              case 7:{
                     recCRC=ch;
                     state=8;
                     break;
                     } 
              case 8:{
                     recCRC=((recCRC<<8)&0xff00)|ch;
                      resultCRC =  calculateCRC16(rbuf,index);  
                     printf("CRC=%02x,%02x\n",recCRC,resultCRC);
                     printf("len:%d\n",len);
                 //    if (recCRC==resultCRC)
                      {
                          data[0]=rbuf[0];
                         data[1]=rbuf[1];
                         if (len>0)
                         for (i=0;i<len;i++)
                           data[i+2]=rbuf[4+i];
                         return len;
                         }
                   //   else {
                   //     for (i=0;i<len;i++)
                   //     cout<<"0x"<<hex<<setfill('0')<<std::setw(2)<<unsigned(rbuf[i])<<endl;
                    //    return len;
                    //    }
                     }                           
             }
         } 
    
        return len;
 }
 unsigned int  serialRPC::calculateCRC16( uint8_t *puchMsg,unsigned int  usDataLen)
{
    unsigned char uchCRCHi,uchCRCLo;
    unsigned char uIndex=0 ;                             /* will index into CRC lookup table */
    unsigned char tempp=0;
    unsigned int CRCHL=0;
    uchCRCHi=0xFF ;                             // high byte of CRC initialized 
    uchCRCLo=0xFF ; 
    tempp=0;                           // low byte of CRC initialized 
    while(usDataLen--)                       // pass through message buffer 
    {
        uIndex=uchCRCHi^*puchMsg++ ; // calculate the CRC high byte         
        tempp++;
        uchCRCHi=uchCRCLo^auchCRCHi[uIndex] ;//01-auchCRCHi[254]:0x81;02-0x01;03-0x80;04-0xC0
        uchCRCLo=auchCRCLo[uIndex] ;               
    }
    CRCHL=uchCRCHi;
    CRCHL=CRCHL<<8;
    CRCHL|=uchCRCLo;        
    return CRCHL;//(uchCRCHi<<8|uchCRCLo) ;//uchCRCHi:0xa1;uchCRCLo:2B
}